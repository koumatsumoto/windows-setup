...
`gr` はprimary worktreeへ移動して `origin` のremote HEADが示す既定branchへ切り替えた後、`grw`、`grb` の順に実行します。これにより、呼び出し元のlinked worktreeを含むlinked worktreeの削除、既定branchの更新、マージ済みlocal branchの削除を1コマンドで行います。未コミット変更などにより既定branchへ安全に切り替えられない場合は、cleanupを開始せず停止します。個別の操作には引き続き `grw` と `grb` を利用できます。
